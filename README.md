...
- **보기**: 프사, 본문 이미지·에셋, 임베드, 이름 표시와 메시지 작업 버튼 접기 선택. **일코 버전 · 노트/전자책**을 켜면 따뜻한 종이색과 읽기용 글꼴로 대화를 표시합니다.
- **···**: 메시지의 번역 등 작업 버튼을 부드럽게 펼칩니다. 펼친 동안에도 점 버튼이 남아 다시 누르면 접힙니다. 바깥을 누르거나 Escape를 눌러도 접힙니다. 연필 수정 버튼은 바로 사용할 수 있도록 남습니다. 기본으로 켜지며 보기에서 해제할 수 있습니다. 기기의 동작 줄이기 설정을 켜면 전환 효과를 생략합니다.
- **Ctrl+Shift+M**: 일코 모드 전환.

상단 왼쪽에는 Note, 조작 버튼은 오른쪽에 표시됩니다. 상단 아이콘은 기능과 툴팁을 유지하는 선 아이콘으로 바뀌고, 브라우저 탭은 문서 아이콘과 메모 제목을 사용합니다. OFF로 돌리면 원래 탭 아이콘과 제목을 복원합니다. 확장 목록에서 Quiet Notes 옆의 계정 ID 배지는 숨깁니다. GitHub 저장소 소유자 자체를 익명화하지는 않습니다.
...
브라우저 확장만으로는 로딩 화면을 바꾸지 않습니다. 첫 화면부터 문서 로딩만 보이게 하려면 실리를 실행하는 서버에서 다음 일회성 설정이 필요합니다.
...
```sh
curl -fL "https://raw.githubusercontent.com/zxvniqi/quiet-notes/main/startup-setup.cjs?v=5" -o "$HOME/quiet-notes-startup.cjs" && node "$HOME/quiet-notes-startup.cjs"
```
...
실리 화면을 새로고침해 확인하세요. 기존 설치자도 위 명령을 다시 실행하면 다음 로딩에까지 나타나던 문서 아이콘이 첫 시작 화면으로만 제한됩니다. 이 설정은 서버의 모든 사용자에게 첫 시작 로딩 화면을 적용하며, 일코 OFF 상태에서도 동일합니다. 앱 설치 아이콘이나 HTML의 초기 탭 제목을 바꾸지는 않습니다.
...
2.5.0: 상단 왼쪽 Note, 오른쪽 조작 버튼, 완전히 숨기는 채팅 프사 틀, 간격을 넓힌 페르소나 아이콘, 펼쳐도 남는 ···, 보기의 일코 버전을 추가했습니다. 시작 로딩은 터묵스 설정에만 남겼고 이후 로딩 아이콘에는 적용하지 않습니다. 블루레몬에이드가 함께 켜져 있으면 일코 ON 동안 그 화면 스타일을 잠시 내리고 OFF에서 복원합니다. 두 확장의 기능은 그대로 로드됩니다.
