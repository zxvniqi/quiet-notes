...
설치 후 새로고침하고 **일코 OFF** 버튼을 누릅니다. OFF 버튼은 입력창 위의 별도 줄에 표시되며 입력창이나 QR을 덮지 않습니다. 버튼이 가려졌다면 확장 설정의 **Quiet Notes · 일코 모드**를 펼쳐 켤 수도 있습니다. 원래 UI 테마를 선택한 상태에서 사용하세요. 이전 Quiet Notes 시험용 태번 헬퍼 스크립트는 꺼 두세요.
...
2.3.2: OFF 버튼은 입력창 위의 독립된 줄을 사용합니다. Chat Lobby가 열리면 일코 도구 줄을 숨기고 닫으면 복원합니다. 일코 입력 부분은 별도 흰 배경이나 테두리 없이 입력 바와 이어집니다.
