...
- **도구**: 기본 상단바와 Chat Top Bar를 함께 클릭으로 접기/펼치기. Chat Top Bar는 한 줄로 표시되며 좁은 화면에서는 가로로 스크롤합니다.
...
- **···**: 메시지의 번역 등 작업 버튼 펼치기. 바깥을 누르거나 Escape를 누르면 접힙니다. 연필 수정 버튼은 바로 사용할 수 있도록 남습니다. 기본으로 켜지며 보기에서 해제할 수 있습니다.
...
2.4.0: 켜기 버튼을 마법봉 메뉴로 옮기고 별도 OFF 버튼 줄을 제거했습니다. Chat Top Bar를 한 줄로 표시하고, 시네마노트에서 치환한 이름이 세로로 보이던 문제를 수정했습니다. 메시지 작업 버튼 접기를 추가했습니다. Chat Lobby가 열리면 일코 도구 줄을 숨기고 닫으면 복원합니다. 일코 입력 부분은 별도 흰 배경이나 테두리 없이 입력 바와 이어집니다.
